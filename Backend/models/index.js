...
const initDB = async (retries = 5, delayMs = 3000) => {
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      console.log(`📡 [Attempt ${attempt}/${retries}] Authenticating and syncing database...`);
  ...
  console.warn('⚠️ Warning: Database connection could not be established after maximum retries. API endpoints will retry dynamically upon request.');
  return false;
};
